...
class Quantization(nn.Module):
    def __init__(self, bit, qq_bit, finetune=False, device='cuda:0'):
        super().__init__()
        self.a_bit = bit
        self.qq_bit = qq_bit
        self.device = device
        if finetune:
            ...
            self.step = gaussian_steps_func(torch.tensor(self.a_bit-1)).to(device)
            self.sig_step = gaussian_steps_func(torch.tensor(self.qq_bit-1)).to(device)
    ...
    def forward(self, x ):
        ...
        if x.min() == 0 :
            ...
            if self.qq_bit!=32:
                mu_sigma = torch.mean(sigma,1,True).detach().to(device)
                sig_sigma = torch.std(sigma,1,True).view(sigma.size(0),1,1,1).detach().to(device)
                
                
                step = self.sig_step* sig_sigma.to(device)
