...
class NGIM(nn.Module):
    def __init__(self,n_feats,scale):
        ...
        if scale == 4:
            ...
            self.TrunkUp = nn.Sequential(
                nn.ConvTranspose2d(n_feats,n_feats,kernel_size=3,stride=3,padding=0),
                nn.PReLU(n_feats)
            )
            self.MultiUp = nn.Sequential(
                nn.Conv2d(n_feats*3,n_feats//2,kernel_size=3,padding=1),
    ...
    def forward(self,xl,xi,xn):
        # print(xl.shape,xi.shape,xn.shape)
        h1 = self.TrunkUp(xi)
...
class GELIN(nn.Module):
    ...
    def forward(self,x,lms):
        ...
        m = []
        device = 'cuda:3'
        y = torch.zeros(b, c,  h*self.scale,  w*self.scale).to(device)
...
if __name__ == "__main__":
    ...
    load = True
    if load:
        model = torch.load('./weight/GELIN_3_Pavnew.pth')
        print("模型读取成功, 进行fine tune 训练！！！")
    else:
        model = GELIN(n_feats=128,n_colors=102,kernel_size=3,pooling_r=2,n_subs=8,n_ovls=2,blocks=6,scale=3).to(device)
    ...
    train_set = HSTrainingData(image_dir= '../PaviaC_mat/train/', n_scale = 3, augment=True, ch3=False, num_ch=0)
    print(len(train_set))
    ...
    OUT_DIR = Path('./weight')
    torch.save(model, OUT_DIR.joinpath('GELIN_3_Pavnew.pth'))